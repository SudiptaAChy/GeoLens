...
import 'package:geolens_capture/features/dashboard/views/batch_sync_progress_header.dart';
...
final class DashboardScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final state = context.watch<UploadQueueCubit>().state;
      ...
      appBar: AppBar(
        ...
        bottom: hasItems
            ? PreferredSize(
                ...
                child: BatchSyncProgressHeader(items: state.items),
              )
            : null,
      ),
      body: Column(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return Padding(
                        padding: EdgeInsets.only(
                          bottom: (index == state.items.length -1) ? 94: 0,
                        ),
                        child: UploadListItem(
                          item: item,
                          onRetry: item.status == UploadStatus.failed
                              ? () => context.read<UploadQueueCubit>().retryItem(
                                  item.id,
                                )
                              : null,
                        ),
